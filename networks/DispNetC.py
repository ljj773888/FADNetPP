...
class ExtractNet(nn.Module):
    ...
    def __init__(self, resBlock=True, maxdisp=192, input_channel=3, encoder_ratio=16, decoder_ratio=16):
        ...
        # weight initialization 代码负责对神经网络的权重进行初始化。它遍历神经网络中的所有模块（self.modules()）
        # ，并对卷积层（nn.Conv2d和nn.ConvTranspose2d）以及批归一化层（nn.BatchNorm2d）的权重和偏置进行初始化。对于卷积层（nn.Conv2d和nn.ConvTranspose2d）：
        #
        # 使用Kaiming正态初始化（kaiming_normal）来初始化权重。Kaiming正态是一种用于初始化权重的方法，有助于深度神经网络的收敛。
        # 如果该层有偏置，则将偏置设置为零（m.bias.data.zero_()）。
        # 对于批归一化层（nn.BatchNorm2d）：
        #
        # 将权重设置为1，偏置设置为零。这是批归一化初始化的常见做法，其中权重最初设置为缩放归一化的输入，偏置设置为移动它们。
